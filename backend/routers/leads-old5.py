...
async def get_top_leads(
    ...
    if not refresh and existing_records:
        # Return all cached leads (active + rejected) — frontend handles display split
        leads = [_record_to_lead(r) for r in existing_records[:10]]
    ...
    if refresh and existing_records:
        ...
        # Take the top 10 records currently shown (by score)
        top_10 = existing_records[:10]
        active_records = [r for r in top_10 if not r.is_rejected]
        slots_needed = max(0, 10 - len(active_records))

        if slots_needed == 0:
            leads = [_record_to_lead(r) for r in top_10]
            return DashboardResponse(
                leads=leads,
        ...
        # Return active existing + new (not rejected ones)
...
async def _fetch_fresh_leads(icp: ICPConfig, exclude_ids: set, limit: int) -> List[Lead]:
    ...
    for company, score in top_scored:
        ...
        leads.append(Lead(company=company, score=score, contact=contact, fetched_at=datetime.utcnow()))
    return leads
...
def _record_to_lead(r: LeadRecord) -> Lead:
    return Lead(
        ...
        is_rejected=r.is_rejected or False,   # NEW — included in response
    )
