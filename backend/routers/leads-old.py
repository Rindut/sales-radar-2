...
from config import DEFAULT_ICP, DUMMY_MODE
...
async def get_top_leads(
    ...
    """
    Get today's Top 5 prioritized leads.
    ...
    if not refresh:
        # Return cached leads from today
        result = await session.execute(
            select(LeadRecord).order_by(desc(LeadRecord.score)).limit(5)
        )
    ...
    if DUMMY_MODE:
        ...
        contact_resolver = lambda cid, roles: get_dummy_contact(cid)
    else:
        raw_companies = await discover_companies(icp, per_page=30)
        contact_resolver = lambda cid, roles: resolve_contact(cid, roles)

    filtered = apply_icp_filter(raw_companies, icp)
    top_scored = score_companies(filtered, icp, top_n=5)

    leads = []
    for company, score in top_scored:
        result = contact_resolver(company.id, icp.target_roles)
        contact = await result if hasattr(result, "__await__") else result
        lead = Lead(
            company=company,
            score=score,
            contact=contact,
...
async def get_lead_detail(
    ...
    # Fallback: fetch fresh from Apollo
