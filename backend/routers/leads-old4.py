...
async def get_top_leads(
    ...
    """
    Get today's leads (up to 10).
    If refresh=true: replace only rejected leads with new ones (Opsi C).
    If not: return cached leads from DB.
    """
    icp = await _get_active_icp(session)

    # Load current leads from DB
    result = await session.execute(
    ...
    if not refresh and existing_records:
        # Return cached — filter out rejected, keep active
        active = [r for r in existing_records if not r.is_rejected][:10]
        rejected = [r for r in existing_records if r.is_rejected]
        leads = [_record_to_lead(r) for r in active] + [_record_to_lead(r) for r in rejected]
        return DashboardResponse(
            leads=leads[:10],
            generated_at=active[0].fetched_at if active else datetime.utcnow(),
            icp_summary=_icp_summary(icp),
        )

    if refresh and existing_records:
        # Opsi C: only replace rejected slots
        active_records = [r for r in existing_records if not r.is_rejected]
        rejected_count = sum(1 for r in existing_records if r.is_rejected)
        slots_needed = max(0, 10 - len(active_records))

        if slots_needed == 0:
            # Nothing rejected, nothing to replace
            leads = [_record_to_lead(r) for r in active_records[:10]]
            return DashboardResponse(
                leads=leads,
        ...
        # Fetch new candidates, exclude companies already in DB
        existing_ids = {r.id for r in existing_records}
        new_leads = await _fetch_fresh_leads(icp, exclude_ids=existing_ids, limit=slots_needed)
        ...
        # Return active (existing) + new leads
        all_leads = [_record_to_lead(r) for r in active_records] + new_leads
        return DashboardResponse(
            leads=all_leads[:10],
            generated_at=datetime.utcnow(),
            icp_summary=_icp_summary(icp),
...
async def _fetch_fresh_leads(icp: ICPConfig, exclude_ids: set, limit: int) -> List[Lead]:
    """Fetch new leads from Apollo, excluding already-known companies."""
    ...
    for company, score in top_scored:
        if DUMMY_MODE and get_dummy_contact:
            contact = get_dummy_contact(company.id)
        else:
